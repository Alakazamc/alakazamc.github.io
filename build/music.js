...
function rows(tracks) {
  return tracks.map(track => `<li class="music-row"><button type="button" class="music-play" data-music-play="${esc(track.id)}" aria-label="试听 ${esc(track.title)}"><span class="music-rank">${track.rank.toString().padStart(2, '0')}</span><svg class="ic music-ic music-ic-play" viewBox="0 0 16 16" aria-hidden="true"><use href="#px-play"></use></svg><svg class="ic music-ic music-ic-pause" viewBox="0 0 16 16" aria-hidden="true"><use href="#px-pause"></use></svg></button><a href="${esc(track.url)}" title="${esc(track.title + ' · ' + track.artist)}" target="_blank" rel="noopener noreferrer"><span class="music-track"><b>${esc(track.title)}</b><small>${esc(track.artist)}</small></span></a></li>`).join('');
}

function render() {
  const data = JSON.parse(fs.readFileSync(path.join(__dirname, 'data/netease.json'), 'utf8'));
  const first = data.week[0];
  return `<div class="record-player" aria-hidden="true"><div class="record-deck"><div class="record-disc"><img class="record-label" ${first ? 'src="' + esc(first.cover + '?param=96y96') + '"' : ''} width="36" height="36" alt="" referrerpolicy="no-referrer" ${first ? '' : 'hidden'}></div><i class="record-arm"></i><i class="record-light"></i></div><div class="record-speaker"></div></div>
  ...
  <p class="music-now" role="status" aria-live="polite">${first ? esc(first.title + ' · ' + first.artist) : '还没有听歌记录'}</p>
  <details class="music-ranking"><summary>听歌排行<span aria-hidden="true">＋</span></summary>
  <div class="music-tabs" role="group" aria-label="音乐收藏与排行">${Object.entries(labels).map(([key, label]) => `<button type="button" data-music-period="${key}" aria-pressed="${key === 'week'}">${label}</button>`).join('')}</div>
...
.record-arm{position:absolute;right:16px;top:10px;width:8px;height:56px;background:#ddd3aa;border:2px solid #695d48;transform:rotate(8deg);transform-origin:top center;box-shadow:2px 2px #333c31;transition:transform .6s steps(5,end)}
...
.music-ranking>summary span{font-size:16px;line-height:16px;transition:transform .18s ease}
...
function paint(){
 ...
 else{ const first=data[period][page*5];now.textContent=first?first.title+' · '+first.artist:'还没有听歌记录'; }
}
